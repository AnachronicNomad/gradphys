...
Data were gathered for the years **2015**-**2022**, accessed on 21 Dec 2023, in Excel spreadsheets available from the ***American Institute of Physics*** (**AIP**); via their [Roster of Physics Departments with Enrollment and Degree Data](https://www.aip.org/taxonomy/term/220).
...
For example, in the 2015 report, data regarding enrollments are the finalized numbers of the 2014-2015 academic year; plus all conferred Bachelors, MS, and PhD certificates from January thru December (inclusive) of the 2015 calendar year.
...
Prior to **2017**, the `Highest Physics Degree Offered` field was not available in the survey.
In our analysis, data were infilled on the condition that if a `Masters` or `PhD` degree certificate was conferred during those years, then the corresponding program at the appropriate level must have existed.
The `Notes` annotations field was removed from this analysis, as the field appeared consistent across years, and after 2019 split into a separate datasheet of the same annual document - the `Notes` values and changelog are available at the source data.
`Institution`-level data were adjusted for varying spellings/canonization over time (e.g. `Appl Phy` -\> `Appl Phys`), and `Coll -> U` if the institution re-designated.
...
Data at the individual observation level (foreach `Institution`, foreach `Year`) ***have not yet been altered*** from their original values.
...
rxtbl <- tdata |>
    reactable(
        ...
        ##  main table
        ####
        elementId = 'rosterphys14_22',
        filterable = T,
        searchable = T,
        groupBy = c('State', 'Highest_Physics_Degree_Offered', 'Institution'),
        bordered = T,
        striped = T,
        highlight = T,
        compact = T,
  ...
  shiny::column(
    ...
    htmltools::browsable(
      tagList(
        tags$button(
          ...
          onclick = "Reactable.downloadDataCSV('rosterphys14_22', 'rosterphys14_22.csv')"
        ),
      )
    ),
    ...
    htmltools::browsable(
      tagList(
        tags$button(
          "Expand/Collapse",
          onclick = "Reactable.toggleAllRowsExpanded('rosterphys14_22')"
        ),
      )
    ),
    ...
    # filter-by state filter_select("st.", "State", tdata, ~State),
    ...
    htmltools::browsable(
        tagList(
            ...
            tags$select(
              ...
              onchange = "Reactable.setGroupBy('rosterphys14_22', this.value ? [this.value] : [])",
              tags$option("None", value = ""),
              lapply(c("State", "Institution", "Highest_Physics_Degree_Offered"), tags$option)
            ),
...
-   A natural boundary to delineate the `r length(unique(rosterdata$Institution))` unique `Institutions` in this set is in the categorical division of `Highest Physics Degree Offered` (`BS`, `MS`, and `PhD`) in the most recent year of the roster publication. We will return to this Bayesian decision boundary of the search space later.
...
-   There is at least one Institution in every state and territory, except the US Virgin Islands, that has a Physics PhD program. There are `TODO` unique `Institutions` offering a `PhD` as of `2022`.
...
-   Applied Physics PhDs are granted by a separate department housed in another College, of Engineering or similar, complementary to an Institution's existing Physics department. Only `6` Institutions of this type (for our analysis) exist: specifically `Stanford`, `Yale`, `Univ. Of Michigan - Ann Arbor`, `Columbia`, `Cornell`, and `Rice` Universities.
...
-   The Top 20 ranking Institutions of cumulative production over the past decade ordered by `PhD`, then followed by `BS` degrees conferred are:
...
-   Has nearby or attached National Lab
